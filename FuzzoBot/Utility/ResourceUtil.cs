...
public static class ResourceUtil
{
    ...
    /// </summary>
    /// <returns></returns>
    public static Task<Dictionary<string, ModTool>> LoadModToolsDictAsync() => LoadResourceDictAsync<string, ModTool>(ModToolsFileName);
    public static Task<Dictionary<string, BotTag>> LoadTagsDictAsync() => LoadResourceDictAsync<string, BotTag>(TagsFileName);
    ...
    private static async Task<Dictionary<T1, T2>> LoadResourceDictAsync<T1, T2>(string fileName) where T1 : class
    {
        ...
        else
        {
            ...
            string url = $"https://raw.githubusercontent.com/CDPR-Modding-Documentation/Cyberpunk-Modding-Docs/main/bot/{fileName}";
        ...
        Dictionary<T1, T2>? toolsDict = await JsonSerializer.DeserializeAsync<Dictionary<T1, T2>>(stream, new JsonSerializerOptions() { WriteIndented = true });
